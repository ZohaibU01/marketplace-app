...
import 'package:toysell_app/MVC/model/chat_message_model.dart';
...
class ProductController extends GetxController {
  ...
  List<ProductModel> get getAllProducts => products
      .where(
        (p) => p.userId != DataStroge.currentUser.value?.id,
      )
      .toList();

  List<ProductModel> get getMyProducts => products
      .where(
        (p) => p.userId == DataStroge.currentUser.value?.id,
      )
      .toList();
  ...
  Future<void> fetchProducts({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final responseData = jsonDecode(response.body)['data']['data'] as List;
  ...
  Future<void> GetSingleChat({
    ...
    try {
      final queryParameters = {
        if (item_Offer_id != null) 'item_offer_id': item_Offer_id.toString(),
        if (type != null) 'type': type.toString(),
        if (item_id != null) 'item_id': item_id.toString(),
      };

      final queryString =
          queryParameters.entries.map((e) => '${e.key}=${e.value}').join('&');
      final response = await ApiService.get('chat-single?$queryString',
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['error'] == false) {
          final messageData = ChatResponse.fromJson(responseData);
          messages.value.add(messageData.data);
          print(
              "messages:${messages.value.length}, item offer id: ${item_Offer_id} ");
          update();
        } else {
  ...
  Future<void> fetchFavoriteProducts() async {
    isLoading.value = true;
    try {
      final response = await ApiService.get('get-favourite-item',
          token: DataStroge.accesstoken.value);
  ...
  void _revertLikeChanges(List<ProductModel> originalProducts,
      List<ProductModel> originalFavorites) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['error'] == false) {
          ...
          final index =
              products.indexWhere((item) => item.id == updatedProduct.id);
          if (index != -1) {
  ...
  Future<void> checkout({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['error'] == false) {
          Get.snackbar(
              "Success", responseData['message'] ?? "Checkout successful");
        } else {
          Get.snackbar("Error",
              responseData['message'] ?? "An error occurred during checkout");
        }
      } else {
        Get.snackbar(
            "Error", "Failed to complete checkout: ${response.reasonPhrase}");
      }
    } catch (e) {
      Get.snackbar("Error", "An error occurred: $e");
    } finally {
  ...
  Future<bool> makeOffer({required int itemId, required double amount}) async {
    try {
      isLoading.value = true;
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['error'] == false) {
          Get.snackbar(
              "Success", responseData['message'] ?? "Offer made successfully");
          return true;
        } else {
          Get.snackbar(
              "Error", responseData['message'] ?? "Failed to make an offer");
          return false;
        }
      } else {
        Get.snackbar(
            "Error", "Failed to make an offer: ${response.reasonPhrase}");
        return false;
      }
    } catch (e) {
      Get.snackbar("Error", "An error occurred: $e");
